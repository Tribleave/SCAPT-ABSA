...
def build_optim(config, model, checkpoint=None):
    """ Build optimizer """
    ...
    if checkpoint is not None:
        ...
        optim = Optimizer(
                config['optimizer'], config['learning_rate'], config['grad_norm'],
                decay_method='noam',
...
def build_optim_bert(config, model, checkpoint=None):
    """ Build optimizer """
    ...
    if checkpoint is not None:
        ...
        optim = Optimizer(
                config['optimizer'], config['learning_rate_bert'], config['grad_norm'],
                decay_method='noam',
                warmup_steps=config['warm_up'],
                weight_decay=config['weight_decay'])
...
class Optimizer(object):
    ...
    def set_parameters(self, params):
        ...
        if self.method == 'sgd':
            ...
            self.optimizer = AdamW(self.params, lr=self.learning_rate, weight_decay=self.weight_decay)
        else:
